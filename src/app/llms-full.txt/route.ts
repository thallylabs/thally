...
async function readRawContent(pageId: string): Promise<string | null> {
  ...
  for (const filePath of candidates) {
    ...
    if (file) {
      ...
      return content
        .replace(/<\/?(?:Steps|Step|Tabs|Tab|Note|Callout|CodeGroup|CodeBlock|CardGroup|Card|Frame|Accordion|Columns|Tooltip|AgentPrompt|Video|YouTube|LiteYouTubeEmbed)[^>]*>/g, '')
        .replace(/\n{3,}/g, '\n\n')
        .trim()
    }
  }
