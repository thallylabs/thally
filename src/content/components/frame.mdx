---
title: "Frame"
description: "A styled image wrapper with optional caption and click-to-zoom."
---

`<Frame>` wraps images and other media in a rounded, bordered container. By default, images are zoomable — click to view at full size.

## Basic usage

<Frame>
  <img src="/images/image1.jpg" alt="Dashboard overview" />
</Frame>

```mdx
<Frame>
  <img src="/images/dashboard.svg" alt="Dashboard overview" />
</Frame>
```

## With caption

<Frame caption="A scenic view — add any descriptive caption below your image.">
  <img src="/images/image2.jpg" alt="Analytics dashboard" />
</Frame>

```mdx
<Frame caption="The analytics dashboard showing daily active users.">
  <img src="/images/analytics.svg" alt="Analytics dashboard" />
</Frame>
```

## Disable zoom

Set `zoom={false}` when the image is purely decorative or you want to prevent the lightbox from opening:

<Frame zoom={false}>
  <img src="/images/image3.jpg" alt="Architecture diagram" />
</Frame>

```mdx
<Frame zoom={false}>
  <img src="/images/diagram.svg" alt="Architecture diagram" />
</Frame>
```

## Light/dark variants

Wrap two images with conditional `className` to swap between light and dark mode:

```mdx
<Frame>
  <img className="block dark:hidden" src="/images/light.svg" alt="Light mode" />
  <img className="hidden dark:block" src="/images/dark.svg" alt="Dark mode" />
</Frame>
```

## Props

| Prop | Type | Default | Description |
|---|---|---|---|
| `caption` | `string` | — | Text shown below the frame |
| `zoom` | `boolean` | `true` | Enable click-to-zoom on the content |
