...
Code blocks are rendered automatically from fenced Markdown syntax. Every
fence must include a language identifier: it selects the syntax grammar and
appears as the block's language tag. Use `<CodeGroup>` to group multiple
language variants into a tabbed view.
...
When a framework name is more useful to readers, add `framework="..."`. The
fence language still controls syntax highlighting, while the framework becomes
the visible tag:

```tsx framework="Next.js" filename="app/page.tsx"
