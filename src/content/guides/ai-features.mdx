...
Thally ships with an AI-native layer that serves both human readers and AI agents from the same URLs. Agents get structured data; humans get the same pre-rendered UI they always did — and most of this layer is free and fully self-hosted.

## Free vs Thally Cloud

Everything an AI agent needs to read and reason about your docs is **free** in the self-hosted template. Grounded chat *answers* — where the site writes prose back to a reader — are a **Thally Cloud** service, because they run an answer pipeline on your behalf.

| Capability | Tier |
|---|---|
| `/llms.txt`, `/llms-full.txt` | Free |
| Structured JSON per page (`/api/docs/{slug}`) & page index | Free |
| `/ai.txt` discovery file | Free |
| JSON-LD structured data | Free |
| Agent Readiness Score | Free |
| MCP tools for querying docs | Free |
| Full-text search | Free |
| **Grounded AI chat answers** | Thally Cloud |
| **Track** (code-triggered docs) & **traffic analytics** | Thally Cloud |

The rest of this page documents the free, agent-facing surfaces. The AI chat widget below is the one Thally Cloud service covered here.

---

## AI chat widget (Thally Cloud)

Thally Cloud adds a floating chat button that lets readers ask questions and get answers grounded in your actual documentation, with inline citations back to the exact pages and anchors.

When enabled, the `/api/chat` route embeds the question, retrieves only the most relevant content chunks (RAG) within a small token budget, and streams back an answer that cites its sources — or says the answer isn't in your docs rather than guessing. In the self-hosted template the widget's panel appears but stays inactive until you connect Thally Cloud.

Enable the widget in `docs.json`, and set the assistant's name and disclaimer live from the [admin dashboard](/guides/admin-dashboard#naming-the-ai-assistant) under **Settings → Site experience**:

```json
{
  "ai": { "chat": true, "label": "Ask Acme" }
}
```
