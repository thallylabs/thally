...
Alongside `/llms.txt`, Dox generates two agent-facing manifests automatically —
...
`docs.json` registration, running `dox check`). It's also the config surface the
`dox agent` reads.
...
Both manifests are linked from `/llms.txt`, and Dox advertises the index on every
