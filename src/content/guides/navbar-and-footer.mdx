...
The top navigation bar and the footer are both configurable from `docs.json`. No code changes required — add a `navbar` or `footer` block and Dox picks it up automatically.
...
When `navbar` is not defined in `docs.json`, Dox falls back to auto-detecting a support link and CTA from `siteConfig.links` in `src/data/site.ts`. Define `navbar` in `docs.json` when you want explicit control.
</Note>
...
When `footer` is not defined in `docs.json`, Dox renders a minimal single-line footer with the site name and `siteConfig.links`.
</Note>
