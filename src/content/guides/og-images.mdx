...
- Display the **site name** and **domain** (from `DOX_SITE_URL`) in the bottom bar
...
| `domain` | Domain shown in bottom bar | Hostname from `DOX_SITE_URL` |
...
Set `DOX_SITE_URL` in your hosting environment. Without it, crawlers resolve the relative `/api/og?...` URL against `localhost`, which is unreachable.
</Note>
