...
When migrating from Mintlify, the `create-dox migrate` command automatically reads the `redirects` field from your Mintlify `docs.json` and emits equivalent entries in the Dox `docs.json`. It also generates redirects for any pages that were renamed during migration.
