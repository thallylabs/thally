...
Dox ships with a built-in **admin dashboard** at `/admin`. It lets you control your docs site at runtime — change branding, toggle features, read analytics, manage your team, and protect the docs behind a password — without editing files or redeploying. Settings chosen here are stored at runtime and layered *on top of* the git-committed defaults in `docs.json` and `src/data/site.ts`.
...
| **Docs tasks** | Documentation edits drafted by the docs agent as pull requests — the [Dox Track](/guides/dox-track) queue. |
...
| **Settings** | Feature toggles, site identity, the AI assistant name & disclaimer, docs-access password, the AI chat key, and **Connect GitHub** for Dox Track. |
...
If you deploy with **neither** `DOX_ADMIN_PASSWORD` nor OIDC set, Dox does not gate `/admin` — anyone who reaches the URL gets full **Owner** access, in production too. There is no localhost-only fallback. Always configure one of the methods below before your site goes public.
</Note>
...
```bash
DOX_ADMIN_PASSWORD=a-long-random-string
```
...
Delegate identity to your own IdP (Google Workspace, Microsoft Entra, or any OIDC provider). Members sign in with their work account, and their **role** comes from a roster you keep in `docs.json`. Nothing is hosted by Dox, so there is no per-seat cost.

Set these environment variables:

```bash
DOX_OIDC_ISSUER=https://accounts.google.com
DOX_OIDC_CLIENT_ID=your-client-id
DOX_OIDC_CLIENT_SECRET=your-client-secret
DOX_AUTH_SECRET=another-long-random-string   # signs sessions & encrypts stored secrets
```
...
Dox uses OIDC discovery and requests the `openid email profile` scopes, with PKCE. On the [Google Cloud console](https://console.cloud.google.com/apis/credentials) or your provider's app registration, create an OAuth client and paste its ID and secret into the variables above.
...
Dox re-checks the roster on every admin page load, so removing or downgrading someone in `docs.json` takes effect on their next request — no waiting for a session to expire.
...
| Logo, favicon | Built-in Dox mark |
...
Locally, analytics and admin state write to on-disk SQLite files under `.data/` — zero config. On **serverless** hosts (for example Vercel), that filesystem does not persist, so Home metrics stay empty and Analytics may show an error until you point Dox at a remote libSQL database (Turso).

Set `DOX_ANALYTICS_DB_URL` / `DOX_ANALYTICS_DB_TOKEN` for traffic data, and `DOX_DATABASE_URL` / `DOX_DATABASE_TOKEN` for durable admin settings and related state. Full setup steps are in [Deploying → Analytics & durable storage](/guides/deploying#analytics--durable-storage).
...
## Connect GitHub (Dox Track)

Under **Settings → Integrations**, an Owner can click **Connect GitHub** to create and install a GitHub App for [Dox Track](/guides/dox-track) — the org-wide access path that lets the docs agent watch selected private repos and draft docs PRs. It needs `DOX_AUTH_SECRET` set (to encrypt the app's key). The status chip shows the connected app and whether it's installed; **Disconnect** clears the stored credentials.
