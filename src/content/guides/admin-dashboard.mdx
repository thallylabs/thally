...
| **Analytics** | Privacy-safe visitors, visits, page views, traffic sources, entry pages, AI readership, search terms, and content gaps. A [Thally Cloud](/guides/ai-features#free-vs-thally-cloud) service — the panel appears locked until you connect Thally Cloud. |
...
<Note title="Free vs Thally Cloud">
The admin panel, search, Agent Readiness, and the [agent endpoints](/guides/ai-features) are free in the self-hosted template. Grounded **AI answers**, **Track** (code-triggered docs), and **traffic analytics** are [Thally Cloud](/guides/ai-features#free-vs-thally-cloud) services — their panels appear locked until you connect Thally Cloud.
</Note>

## How traffic metrics are counted

Thally keeps people and AI traffic separate so an indexing crawl cannot make
your human readership look larger. Known AI agents are reported under **AI
tools**; ordinary search crawlers and synthetic monitoring are excluded.

Human analytics use no cookies or persistent browser storage:

- **Visitors** are daily anonymous uniques. The runtime HMACs the request IP
  and User-Agent with a site-scoped secret and UTC date, sends only the HMAC,
  and never stores either raw value. The identifier changes the next day.
- **Visits** group one visitor's page views until there has been 30 minutes of
  inactivity.
- **Page views** count full loads and client-side navigation, while speculative
  prefetches and admin traffic are excluded.
- **Bounce rate** is the percentage of visits with exactly one page view.
- **Traffic sources** retain only the external referring hostname. Referrer
  paths, query strings, and same-site navigation are discarded.

After upgrading an existing site, historical page views remain visible but do
not have anonymous visitor keys. The dashboard shows identity coverage whenever
visitor and visit metrics cover only part of the selected range.
...
Set the password and sign in with it. Anyone who knows it gets **Owner** access — best for a solo maintainer or a quick setup. Production also requires an independent session-signing secret.

```bash
THALLY_ADMIN_PASSWORD=a-long-random-string
THALLY_ADMIN_SECRET=an-independent-random-signing-secret
```

Generate the signing secret with `openssl rand -hex 32` and save it in your
host's secret manager. Keep it stable across deployments. Changing it signs
out existing password sessions; production login returns `503` if it is absent.

<Warning title="Keep it strong">
...
Self-hosted production sites also need `THALLY_ACCESS_SECRET`, generated with
`openssl rand -hex 32`. If it is absent, the runtime uses `THALLY_ADMIN_SECRET`;
it never uses the visitor password as a signing key. Configure the secret before
enabling password access. Managed hosting provisions the signing key when it
publishes the site.

Password endpoints include a bounded, per-instance attempt limiter before
password verification. Serverless instances and Workers do not share that
memory globally, so production deployments should also configure a durable
provider-side WAF or rate-limit rule for `POST /api/admin/auth` and
`POST /api/access/auth`. Treat the application limiter as defense in depth,
not as a replacement for an edge rule shared across every instance.
...
Locally, admin state such as settings overrides and rate-limit counters writes
to an SQLite file under `.data/` without additional setup. On **serverless**
hosts such as Vercel, that filesystem does not persist. Configure a remote
libSQL database such as Turso with `THALLY_DATABASE_URL` and
`THALLY_DATABASE_TOKEN` before you depend on durable runtime settings. See
[Deploying → Durable admin storage](/guides/deploying#durable-admin-storage).
...
The AI chat widget is a [Thally Cloud](/guides/ai-features#free-vs-thally-cloud) service, so its answers light up only when a deployment carries a Thally Cloud grant. Its *appearance*, however, is a free admin setting: under **Settings → Site experience** you can rename the assistant and set its disclaimer, live:
