...
<Update label="Dox Track: org access & preview" date="2026-07-10">
...
- **Self-tracking loop guard** — a repo can safely track itself; the agent's own `dox/agent-*` PRs never re-trigger Track.
- Unified GitHub token resolution across the webhook, MCP tool, and tasks queue: explicit token → GitHub App → PAT chain.

See [Dox Track](/guides/dox-track).
...
- The **docs agent** drafts documentation edits as reviewed pull requests from a `@dox` comment, a merge, the drift sweep, or the CLI — it never merges.
