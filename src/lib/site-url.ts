...
/**
 * Guard against a stale `DOX_SITE_URL`: when the configured site-URL host does
...
export function siteUrlMismatch(requestOrigin: string): string | null {
  ...
  const message = `Configured site URL host (${configuredHost}) does not match the request origin (${requestHost}); absolute links in llms.txt, sitemap.xml, and JSON-LD point at ${configuredHost}, which may be unreachable. Set DOX_SITE_URL to your real origin.`
  if (!warnedMismatch) {
    warnedMismatch = true
    console.warn(`[dox] ${message}`)
  }
  return message
}
