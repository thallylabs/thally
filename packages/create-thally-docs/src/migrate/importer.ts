...
function normalizeComponents(body: string): string {
  ...
  // Snippet imports like `import X from '/snippets/foo.mdx'` can't resolve in
  // Thally's rendering pipeline. Strip them and stub any JSX usage.
  ...
  // These power Mintlify's interactive API Playground — Thally uses OpenAPI specs.
  // Convert to bold field name + type metadata so content is still readable.
  ...
  // Mintlify pins these to the right sidebar — Thally renders inline.
  result = result.replace(/<RequestExample[^>]*>/g, '<CodeGroup>')
  result = result.replace(/<\/RequestExample>/g, '</CodeGroup>')
  result = result.replace(/<ResponseExample[^>]*>/g, '<CodeGroup>')
  result = result.replace(/<\/ResponseExample>/g, '</CodeGroup>')

  // ── 11. <Panel> → strip wrapper (Mintlify sidebar panel, no Thally equivalent) ─
  ...
  // ── 19. <Banner> → strip (site-wide bar configured in docs.json, not MDX) ─
