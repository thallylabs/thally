 ...
 */
export const TRACK_OWNED_BRANCH_PREFIXES = [
  AGENT_BRANCH_PREFIX,
  "thally/track-vnext/",
] as const;

/** Return whether a branch or full `refs/heads/` ref belongs to Track. */
export function isTrackOwnedBranch(ref: string | null | undefined): boolean {
  if (typeof ref !== "string") return false;
  const branch = ref.startsWith("refs/heads/")
    ? ref.slice("refs/heads/".length)
    : ref;
  return TRACK_OWNED_BRANCH_PREFIXES.some((prefix) =>
...
export async function mintInstallationToken(
  ...
  const keyFp = createHash("sha256")
    .update(creds.privateKey)
    .digest("hex")
    .slice(0, 12);
...
function envAppCreds(): GithubAppCreds | undefined {
  const appId = (
    process.env.THALLY_GITHUB_APP_ID ?? process.env.DOX_GITHUB_APP_ID
  )?.trim();
  const installationId = (
    process.env.THALLY_GITHUB_APP_INSTALLATION_ID ??
    process.env.DOX_GITHUB_APP_INSTALLATION_ID
  )?.trim();
  const privateKey =
    process.env.THALLY_GITHUB_APP_PRIVATE_KEY ??
    process.env.DOX_GITHUB_APP_PRIVATE_KEY;
...
export async function fetchPullRequest(
  ...
  const raw = await githubJson<RawPull>(
...
export function buildTrackContext(
  ...
  for (const file of files) {
    ...
    if (
      ...
      TRACK_CONTEXT_CHAR_CAP - NOTE_RESERVE
    ) {
