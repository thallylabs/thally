...
describe("OpenAPI authoring tools", () => {
  it("reads and validates the one configured API source", async () => {
    ...
    await expect(handleReadApiSpec({ projectDir })).resolves.toContain(
      "openapi: 3.1.0",
    );
    await expect(
      handleUpdateApiSpec({
    ...
    expect(readFileSync(join(projectDir, "openapi.yaml"), "utf8")).toContain(
      "/v1/events",
    );
  });

  it("returns large sources through exact bounded continuations", async () => {
    const projectDir = project();
    const source = `openapi: 3.1.0\n# ${"é".repeat(100_000)}\npaths: {}\n`;
    writeFileSync(join(projectDir, "openapi.yaml"), source);

    const first = await handleReadApiSpec({ projectDir, maxBytes: 180 * 1024 });
    ...
    const second = await handleReadApiSpec({
      projectDir,
      startByte: next,
      maxBytes: 180 * 1024,
    });
    expect(second).toContain("complete: true");
    expect(first.match(/content-sha256: ([a-f0-9]{64})/)?.[1]).toBe(
      second.match(/content-sha256: ([a-f0-9]{64})/)?.[1],
    );
  });

  it("rejects invalid UTF-8 rather than replacing bytes inside a source window", async () => {
    const projectDir = project();
    writeFileSync(
      join(projectDir, "openapi.yaml"),
      Buffer.from([0x61, 0xc3, 0x28]),
    );
    await expect(handleReadApiSpec({ projectDir })).rejects.toThrow(
  ...
  it("rejects invalid documents and paths outside docs.json", async () => {
    ...
    await expect(
      handleUpdateApiSpec({ projectDir, content: "not: openapi\n" }),
    ).rejects.toThrow("openapi and paths");
    await expect(
      handleUpdateApiSpec({
        projectDir,
        source: "../package.json",
