...
describe("parseOwnerRepo", () => {
  it("parses owner/repo", () => {
    expect(parseOwnerRepo("acme/api-server")).toEqual({
      owner: "acme",
      repo: "api-server",
    });
  });
  it("parses owner/repo#123 (PR number)", () => {
    expect(parseOwnerRepo("acme/api#42")).toEqual({
      owner: "acme",
      repo: "api",
      pr: 42,
    });
  ...
  it("captures a PR number from a /pull/<n> URL", () => {
    expect(parseOwnerRepo("https://github.com/acme/api/pull/42")).toEqual({
      owner: "acme",
      repo: "api",
      pr: 42,
    });
    // a bare repo URL has no pr
    expect(parseOwnerRepo("https://github.com/acme/api.git")).toEqual({
      owner: "acme",
      repo: "api",
    });
  });
...
describe("Track-owned branch namespaces", () => {
  it("recognizes every writer namespace in branch and push-ref form", () => {
    expect(TRACK_OWNED_BRANCH_PREFIXES).toEqual([
      "thally/agent-",
      "thally/track-vnext/",
    ]);
    for (const branch of [
      "thally/agent-abc123",
      "thally/track-vnext/0123456789abcdef",
    ]) {
      expect(isTrackOwnedBranch(branch)).toBe(true);
      expect(isTrackOwnedBranch(`refs/heads/${branch}`)).toBe(true);
    }
  });

  it("does not treat lookalike customer branches as Track-owned", () => {
    expect(isTrackOwnedBranch("feature/thally/track-vnext/change")).toBe(false);
    expect(isTrackOwnedBranch("thally/track-vnext")).toBe(false);
    expect(isTrackOwnedBranch("thally/agent")).toBe(false);
    expect(isTrackOwnedBranch(null)).toBe(false);
  });
});
...
describe("filterFilesByGlobs", () => {
  ...
  it("filters by any-of globs", () => {
    expect(
      filterFilesByGlobs(files, ["src/**", "openapi.yaml"]).map(
        (f) => f.filename,
      ),
    ).toEqual(["src/a.ts", "openapi.yaml"]);
  });
});
...
describe("fetchPullRequest / fetchPullRequestFiles / fetchLatestMergedPr", () => {
  it("fetches PR metadata", async () => {
    ...
    const pr = await fetchPullRequest("a", "b", 42, {
      fetchImpl: impl,
      token: "t0k",
    });
  ...
  it("fetches PR files with patches", async () => {
    ...
    const files = await fetchPullRequestFiles("a", "b", 42, {
      fetchImpl: impl,
    });
  ...
  it("returns null when there is no merged PR", async () => {
    const { impl } = fakeFetch(200, [
      { number: 9, merged_at: null, base: { ref: "main" } },
    ]);
    expect(
      await fetchLatestMergedPr("a", "b", "main", { fetchImpl: impl }),
    ).toBeNull();
  });
  ...
  it("throws a hint on 404", async () => {
    ...
    await expect(
      fetchPullRequest("a", "b", 1, { fetchImpl: impl }),
    ).rejects.toThrow(/THALLY_GITHUB_TOKEN/);
  });
});
...
describe("buildTrackInstruction / buildTrackTask", () => {
  ...
  it("reframes the instruction for preview (open) PRs, still no shell-breaking quotes", () => {
    const preview = buildTrackInstruction({ owner: "acme", repo: "api" }, pr, {
      preview: true,
    });
...
describe("mintInstallationToken / resolveGithubToken", () => {
  it("signs an RS256 JWT, exchanges it, and caches the installation token", async () => {
    ...
    const fetchImpl = (async (_url: unknown, init?: unknown) => {
      calls++;
      sentAuth = (init as RequestInit).headers
  ...
  it("is true only when the installation's app_id matches (CSRF guard for the install callback)", async () => {
    ...
    const match = (async () =>
      ({
        ok: true,
        status: 200,
        json: async () => ({ app_id: 42 }),
      }) as Response) as typeof fetch;
    expect(await verifyInstallationBelongsToApp(42, key, 999, match)).toBe(
      true,
    );
    ...
    expect(await verifyInstallationBelongsToApp(42, key, 999, foreign)).toBe(
      false,
    );
    ...
    expect(
      await verifyInstallationBelongsToApp(42, key, 123456, notFound),
    ).toBe(false);
  });
});
