...
function normalizeComponents(body: string): string {
  ...
  // Dox's rendering pipeline. Strip them and stub any JSX usage.
  ...
  // <Tip>   → <Note> (positive suggestion, no dedicated Dox variant)
  ...
  // These power Mintlify's interactive API Playground — Dox uses OpenAPI specs.
  ...
  // Mintlify pins these to the right sidebar — Dox renders inline.
  result = result.replace(/<RequestExample[^>]*>/g, '<CodeGroup>')
  result = result.replace(/<\/RequestExample>/g, '</CodeGroup>')
  result = result.replace(/<ResponseExample[^>]*>/g, '<CodeGroup>')
  result = result.replace(/<\/ResponseExample>/g, '</CodeGroup>')

  // ── 11. <Panel> → strip wrapper (Mintlify sidebar panel, no Dox equivalent) ─
