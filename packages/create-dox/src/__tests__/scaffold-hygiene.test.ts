...
// A scaffold must NOT inherit the Dox project's own project-specific wiring.
// These guards are the safety net: if a future template change re-introduces the
// Dox project's tracking/agent config into a scaffold, one of these fails.
describe('scaffold hygiene — Track/agent are opt-in, never inherited', () => {
  describe('download filter (shouldInclude)', () => {
    it('excludes the monorepo tooling and the opt-in Track/agent wiring', () => {
      // Tarball entries look like `Dox-main/<path>`.
      expect(shouldInclude('Dox-main/packages/mcp/src/lib/track.ts')).toBe(false)
      expect(shouldInclude('Dox-main/cli/index.ts')).toBe(false)
      expect(shouldInclude('Dox-main/.github/workflows/dox-agent.yml')).toBe(false)
      expect(shouldInclude('Dox-main/.github/workflows/dox-track.yml')).toBe(false)
      expect(shouldInclude('Dox-main/.github/CODEOWNERS')).toBe(false)
    })
    ...
    it('still includes the platform source, docs.json, and generic CI', () => {
      ...
      // only the Dox project's own config/workflows are stripped.
      expect(shouldInclude('Dox-main/src/app/api/track/webhook/route.ts')).toBe(true)
      expect(shouldInclude('Dox-main/src/lib/track/github-app.ts')).toBe(true)
      expect(shouldInclude('Dox-main/src/components/admin/github-connect-panel.tsx')).toBe(true)
      expect(shouldInclude('Dox-main/docs.json')).toBe(true)
      expect(shouldInclude('Dox-main/.github/workflows/ci.yml')).toBe(true)
    })
  })
  ...
  describe('resetTrackingConfig', () => {
    ...
    it("drops the template's tracking block but preserves the rest of docs.json", () => {
      dir = mkdtempSync(join(tmpdir(), 'dox-scaffold-'))
      writeFileSync(
        join(dir, 'docs.json'),
        JSON.stringify({
          tabs: [{ tab: 'Docs' }],
          ai: { chat: true },
          tracking: { repos: [{ owner: 'kenny-io', repo: 'Dox', branch: 'main' }] },
        }),
      )
