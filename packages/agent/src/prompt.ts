...
export function buildSystemPrompt(agentsGuidance: string): string {
  const base = [
    ...
    "- When the task context supplies evidence reference IDs and citation markers, copy only the exact",
    "  applicable marker into the drafted MDX and submit factualClaims for every added non-empty prose",
    ...
    "  metadata: follow next-start-byte until complete, and never replace a file from a partial window.",
